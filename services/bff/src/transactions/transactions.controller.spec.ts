...
describe('BFF TransactionsController (proxy)', () => {
  ...
  it('places overdue before :id so the path resolves', async () => {
    const api = makeApi();
    const controller = new TransactionsController(api);
    await controller.overdue(makeReq(), { before: '2026-01-01T00:00:00.000Z' });
    expect(api.get).toHaveBeenCalledWith(
  ...
  it('appends scope to update and forwards Idempotency-Key', async () => {
    ...
    await controller.update(makeReq(), 't1', body, 'future', 'idem-2');
  ...
  it('forwards effectuate to the API-MS', async () => {
    ...
    await controller.effectuate(makeReq(), 't1', body, 'idem-3');
  ...
  it('appends scope to delete', async () => {
    const api = makeApi();
    const controller = new TransactionsController(api);
    await controller.remove(makeReq(), 't1', 'all', 'idem-4');
